...
## Two remotes, and `19.0-marin` is on neither
...
The fork carries **`19.0` only**, in sync with upstream. `19.0-marin` has never
been pushed to it, so every commit on the active line exists on one disk. This is
not theoretical: the `theme_common` snippet port that `odoo`'s `6a7e4c38dd8`
recorded in its JS surface pins (design-themes `b76c148db`) is in this
repository's history, its reflog, `git fsck` and the fork — in none of them. It
was lost precisely because the branch holding it was never pushed. Push
`19.0-marin` and the exposure closes.
