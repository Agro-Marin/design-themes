...
CONFLICTUAL_CLASSES_RE = {
    ...
    # Font awesome
    re.compile(r"^fa-\dx$"): [],
    re.compile(r"^fa-(?:solid|regular|brands)$"): [],
    # Whitelist workaround for s_social_media inner snippet Layout: None
    re.compile(r"^fa-...+"): ["fa-stack", "fa-solid", "fa-regular", "fa-brands"],
    # Rounded
...
S_CLASSES_WHITELIST = [
    ...
    "s_blog_post_list",
    "s_blog_post_single_aside",
    "s_blog_post_single_full",
    "s_blog_post_single_circle",
    "s_blog_post_single_badge",
    "s_event_event_picture",
    "s_event_event_card",
    "s_event_event_single_card",
    "s_event_event_single_entry",
    "s_event_event_single_offset",
    "s_event_event_single_badge",
    "s_event_event_single_aside",
    "s_appointment_type_card",
    "s_appointment_type_picture",
    "s_appointment_type_list",
    "s_newsletter_list",
...
class TestNewPageTemplates(TransactionCase):
        ...
        for website in websites_themes:
            ...
            if website.theme_id.name != "theme_default":
                self.assertGreater(
                    len(views),
                    10,
                    "Test should have encountered some views in theme %r"
                    % website.name,
                )
            for view in views:
                self.assertEqual(
                    view.mode,
                    "extension",
                    "Theme's new page template customization %r should never be primary"
                    % view.key,
                )
                ...
                self.assertEqual(
                    name,
                    parent_name,
                    "Theme's new page template customization %r should use the same name as their parent %r"
                    % (view.key, view.inherit_id.key),
                )
    ...
    def test_render_templates(self):
        ...
        self.assertGreater(
            len(view_ids), 1250, "Test should have encountered a lot of views"
        )
    ...
    def test_attribute_separator(self):
        ...
        for module_name in [
            "website",
            *(
                website.theme_id.name
                for website in self.env["website"].get_test_themes_websites()
            ),
        ]:
            views = View.search(
                [
            ...
            for view in views:
                try:
                    ...
                    _logger.error(
                        "Using %r, view %r cannot be parsed: %r",
                        module_name,
                        view.key,
                        view.arch_db,
                    )
                    errors.append(
                        "Using %r, view %r cannot be parsed: %r"
                        % (module_name, view.key, view.arch_db)
                    )
                    continue
